...
class HMM:
    ...
    def alpha(self, x, u, j):
        ...
        return sum([self.alpha(x,v,j-1) * self.a[v][u] * self.e(v,x[j]) for v in self.tags])
    
    @lru_cache(maxsize=None)
    def beta(self, x, u, j):
        if j == len(x)-1:
            return self.a[u]['STOP'] * self.e(u,x[j])
        return sum([self.a[u][v] * self.e(v,x[j]) * self.beta(x,v,j+1) for v in self.tags])
    ...
    def mmd(self, x):
        ...
        for i in range(len(x)):
            ...
            for tag in self.tags:
                ...
                #if score != 0: print(tag,score)
                if score > opt_score:
                    opt_score = score
                    opt_y[i] = tag
                    #print(x,i,opt_y[i],score)
        return opt_y
